#!/usr/bin/env bun
/**
 * gitlab-standards channel server — stdio MCP server implementing the channel contract.
...
const mcp = new Server(
  { name: 'gitlab-standards', version: '0.1.0' },
  {
    capabilities: {
      tools: {},
    ...
    instructions:
      "Events from gitlab-standards arrive as <channel source=\"gitlab-standards\" ...>. Anything " +
...
mcp.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    {
      name: 'reply',
      description: 'Send a message back to the gitlab-standards channel.',
      inputSchema: {
        type: 'object',
